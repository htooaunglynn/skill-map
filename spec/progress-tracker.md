...
- GitHub community files added for the public repository: MIT `LICENSE`, `CODE_OF_CONDUCT.md`, `CONTRIBUTING.md`, `SECURITY.md`, issue templates, and package/README description metadata.
- Global atmospheric background added across the public landing page and planner app shells, matching the provided dark reference with layered teal/blue/rose/amber glows, a subtle dot texture, and faint line/ring texture. `pnpm lint` and `pnpm build` pass.
- Landing Motion and global hover polish implemented: added the `motion` package, animated the landing-page privacy promise, added shared hover/tap animation surfaces for landing cards and planner item cards, added a global clickable hover baseline with reduced-motion handling, and documented global CSS plus animation package usage in `spec/ui-context.md`.
- SkillMap brand system implemented: added scalable map/pin logo, wordmark, compact planner brand header, privacy shield icon, SVG app icon metadata, and Open Graph wordmark treatment. Brand usage rules were documented in `spec/ui-context.md`.
...
- Public repository community setup completed: added contributor guidance aligned with the spec-first workflow and local-first constraints, a concise code of conduct, an MIT license, a security policy focused on local file/IndexedDB/Markdown risks, GitHub issue forms for bugs/features/docs, and repository description metadata in `package.json`/`README.md`.
- Global background polish completed: added a reusable `skillmap-background` utility in `app/globals.css`, applied it to the landing page, planner pages, and dashboard setup state, and added a base body gradient so footer/uncovered space remains visually consistent. Browser checks confirmed the radial background, dot layer, and line/ring layer render on `/`, `/app`, desktop, and mobile with no horizontal overflow.
- Landing Motion and hover polish completed: installed `motion`, added small client-side Motion components for the public landing privacy statement and animated surfaces, applied shared hover treatment to landing and planner cards, and extended `app/globals.css` with clickable hover transitions plus a reduced-motion override. `spec/ui-context.md` now documents the global CSS layers, `.skillmap-background`, `.skillmap-motion-card`, scrollbar styling, clickable hover conventions, and the split between `gsap` planner animations and `motion` interaction animations.
- Brand logo pass completed: created reusable SkillMap brand components for the map/pin mark, wordmark header, and shield/check privacy icon; added the logo to the landing page and planner page headers; added `app/icon.svg` and metadata icon references; and added brand identity to generated social images.
